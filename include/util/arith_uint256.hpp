...
// Forward declare arith_uint512 for use in arith_uint256
class arith_uint512;
...
// UNICITY EXTENSION: 512-bit unsigned big integer support
...
public:
    ...
    /** Construct from arith_uint256 (extends to 512 bits) */
    arith_uint512(const arith_uint256& b) : base_uint<512>()
    {
        // Copy the 256 bits (32 bytes) from the source
        memcpy(this->pn, &b, 32);
        // Zero out the upper 256 bits
        memset(this->pn + 8, 0, 32);
    }
};

static_assert(std::is_trivially_copyable_v<arith_uint512>);

extern template class base_uint<512>;

/** Convert arith_uint512 to arith_uint256 (truncates to lower 256 bits) */
inline arith_uint256 ArithU512ToU256(const arith_uint512& u512)
{
    arith_uint256 result;
    // Copy the lower 256 bits (32 bytes)
    memcpy(&result, &u512, 32);
    return result;
}

...
// Copyright (c) 2025 The Unicity Foundation developers
...
template <unsigned int BITS> std::string base_uint<BITS>::GetHex() const {
  ...
  for (int x = 0; x < this->WIDTH; ++x) {
    endian::WriteLE32(b.begin() + x * 4, this->pn[x]);
  }
  return b.GetHex();
}
...
template <unsigned int BITS> void base_uint<BITS>::SetHex(const char *psz) {
  // Parse hex string and set value
  base_blob<BITS> b;
  b.SetHex(psz);

  // Convert from base_blob to base_uint (read little-endian)
  for (int x = 0; x < WIDTH; ++x) {
    pn[x] = endian::ReadLE32(b.begin() + x * 4);
  }
}
...
uint256 ArithToUint256(const arith_uint256 &a) {
  ...
  for (int x = 0; x < a.WIDTH; ++x)
    endian::WriteLE32(b.begin() + x * 4, a.pn[x]);
  return b;
}
arith_uint256 UintToArith256(const uint256 &a) {
  arith_uint256 b;
  for (int x = 0; x < b.WIDTH; ++x)
    b.pn[x] = endian::ReadLE32(a.begin() + x * 4);
  return b;
}

// UNICITY EXTENSION: Explicit instantiations for base_uint<512>
